...
description: >
  ...
  verify-docs の重複検査（scripts/verify-docs.mjs）はAST抽出した段落本文の
  完全一致（強調などの書式差は無視。準一致も句読点・語尾の機械的な正規化のみ）しか見ないため、言い回しが
...
verify-docs の重複検査はAST抽出した段落本文の一致だけを見る。強調などの書式差は
吸収するが、言い換えによる重複は素通りする。本スキルはこの隙間を人・エージェントの
判断で埋める。
