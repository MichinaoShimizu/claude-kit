...
- **挙動を変更した場合、`SKILL.md`・`references/config.md`・README.md の
  ...
  `references/config.md` の表への追記を `verify-docs.test.mjs`
  （「every DEFAULTS key is documented in config.md」）が機械的に検査する。**
  README.md 側は個々のキー名を列挙しない（config.md のみを単一の
  情報源とし、2箇所に同じ一覧を持たない）
- **新規検査・オプションを追加する前に、まず [TODO.md](TODO.md) に
  意図と理由を記載する。** 「機械的に真偽が判定できるか」を超える判断
  （表記ゆれの許容、意味的な類似判定など）は、実装の是非を保留のまま
  記録する（[TODO.md](TODO.md) の重複検査の項を参照）
