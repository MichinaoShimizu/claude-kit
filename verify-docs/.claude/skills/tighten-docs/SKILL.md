...
典型パターン。各見出しの下に定義と例を記す。

#### 見出し直後の言い換え文

見出しをそのまま言い換えているだけで情報を追加していない一文が、見出し
直後に置かれている状態。

#### 体言止め

見出しが「〜する」で終わる動詞句のままになっている状態。可能な限り
名詞句に言い換える。省略できる助詞（の・を等）は省略して複合名詞にする。
目次を眺めたときにページ全体の構造を把握しやすくなる。
...
同じ理由を複数回言い換えて説明している状態。

#### 強調の重ね置き

強調のための太字・繰り返しが、説明の直後にも重ねて置かれている状態。
...
分岐が実質1つしかないのに毎回明示している条件節。

- 「〜の場合には」

#### 重複例示

同じ論点を示す例示が複数列挙されていて1つで足りる状態。

#### 「〜を行う」表現

サ変名詞に「〜を行う」を続けているだけの表現。

- 「テストを行う」→「テストする」

#### 同義語の重ね書き

漢語と和語で同じ意味の動詞を並べているだけの表現（同義語2語を「・」で
並べる形。下記「重言」は1つの複合語の中に意味の重複が埋め込まれている
点で異なる）。
...
括弧内の注釈が本文の単純な言い換えで、本文だけで意味が通じる状態。
...
意味を変えずに肯定形へ言い換えられる否定の否定。

- 「〜ないわけではない」→「〜である」

#### 語句の連続誤字

誤字起因で同一語句が連続しているだけの箇所。
...
表現（上記「同義語の重ね書き」との違いは上記参照）。

- 「まず最初に」→「まず」
- 「後で後悔する」→「後悔する」
- 「今の現状」→「現状」
- 「従来から」→「従来」
- 「頭痛が痛い」→「頭痛がする」（頭痛は元々「痛み」の意味を含む）
- 「違和感を感じる」→「違和感がある」（違和感は元々「感じる」の意味を含む）
- 「馬から落馬する」→「落馬する」（落馬は元々「馬から落ちる」の意味を含む）
