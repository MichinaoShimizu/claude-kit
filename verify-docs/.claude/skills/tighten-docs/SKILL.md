...
- 意味を変えずに肯定形へ言い換えられる二重否定（「〜ないわけではない」→
  「〜である」）
- 誤字起因で同一語句が連続しているだけの箇所（「〜のの」「〜するする」）
- 概数表現を二重に示しているだけの箇所（「約100個程度」→「約100個」）
- 重言になっている箇所（「まず最初に」→「まず」、「従来から」→「従来」）

#### 用語メモ

- **重言（じゅうげん）**: 一方の語がもう一方の意味を既に含んでいるのに
  両方を続けて使っている複合表現（例:「後で後悔する」「今の現状」）。
  漢語と和語の重ね書き（別項目）は同義語2語を「・」で並べる形だが、
  重言は1つの複合語の中に意味の重複が埋め込まれている点で異なる
