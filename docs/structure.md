...
Markdown文書のリポジトリ内参照の整合性・文書サイズ・段落重複を検査する。外部 URL は
アクセスせず、検査対象にしない。
...
| 文書構造検証器 | リポジトリ内参照の切れ、孤立文書、**ファイル単位**のサイズ超過、AST抽出した同一段落の重複を検出する。外部 URL はネットワークアクセスを伴うため検査しない。違反箇所の行・列・見出し階層、文書・違反種別の集計、分割検討用の大きい葉セクションも出力する。 |
